...
/**
 * Whether the schema currently being emitted sits in an INPUT position.
 *
 * Optional properties admit `undefined` only on inputs. JSON has no
 * `undefined`, so on the way out `?: T` and `?: T | undefined` describe the
 * same wire value — but they are NOT interchangeable for a consumer under
 * `exactOptionalPropertyTypes`: widening a RESPONSE property breaks assigning
 * the response into the consumer's own interface, breaks `"k" in obj`
 * narrowing, and breaks `Required<T>`. Widening an INPUT property costs
 * nothing and unblocks the ordinary `html_content: template.html` case.
 *
 * `$ref`s are emitted as references rather than expanded, so this flag scopes
 * exactly to the schema being written plus its inline sub-objects.
 */
let emittingInputSchema = false;

function requestSideSchemaNames(document, schemas) {
  const seeds = new Set();
  for (const pathItem of Object.values(assertRecord(document.paths, "paths"))) {
    for (const operation of Object.values(assertRecord(pathItem, "path item"))) {
      if (operation === null || typeof operation !== "object" || Array.isArray(operation)) continue;
      const body = operation.requestBody;
      if (body === null || typeof body !== "object") continue;
      const media = body.content?.[JSON_CONTENT_TYPE];
      if (media?.schema === undefined) continue;
      for (const name of collectSchemaReferences(media.schema)) seeds.add(name);
    }
  }
  ...
  while (pending.length > 0) {
    ...
    if (name === undefined || reachable.has(name) || schemas[name] === undefined) continue;
    reachable.add(name);
    for (const next of collectSchemaReferences(schemas[name])) pending.push(next);
  }
  return reachable;
}

function collectSchemaReferences(node, found = new Set()) {
  if (node === null || typeof node !== "object") return found;
  if (Array.isArray(node)) {
    for (const item of node) collectSchemaReferences(item, found);
    return found;
  }
  for (const [key, value] of Object.entries(node)) {
    if (key === "$ref") {
      const name = schemaNameFromReference(value);
      if (name !== undefined) found.add(name);
    } else {
      collectSchemaReferences(value, found);
    }
  }
  return found;
}
  ...
  if (
    ...
    const fields = propertyNames.map((name) => {
      ...
      const optionalSuffix =
        !emittingInputSchema ||
        fieldType === "unknown" ||
        fieldType.split("|").some((part) => part.trim() === "undefined")
          ? ""
          : " | undefined";
...
function operationType(document, entry, componentSchemas) {
  ...
  if (entry.operation.requestBody !== undefined) {
    ...
    lines.push(
      (() => {
        emittingInputSchema = true;
        const bodyType = schemaType(media.schema, 3, undefined, componentSchemas);
        emittingInputSchema = false;
        return `    requestBody${requestBody.required === true ? "" : "?"}: { content: { \"application/json\": ${bodyType} } };`;
      })(),
    );
  }
